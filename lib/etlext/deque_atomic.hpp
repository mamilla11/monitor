...
///\ingroup deque
//***************************************************************************
class deque_atomic_base
{
public:

  typedef etl::atomic_size_t size_type;
...
protected:
  ...
  /// Constructor.
  //*************************************************************************
  deque_atomic_base(size_t max_size_, size_t buffer_size_)
    : current_size(0),
      CAPACITY(max_size_),
...
//***************************************************************************
template <typename T>
class ideque_atomic : public etlext::deque_atomic_base
{
public:
...
public:
  ...
  struct iterator : public std::iterator<std::random_access_iterator_tag, T>
  {
    ...
    {
      return ideque_atomic::distance(*this, other) > 0;
    }
  ...
  private:
    ...
    //***************************************************
    iterator(difference_type index_, ideque_atomic& the_deque, pointer p_buffer_)
      : index(index_),
        p_deque(&the_deque),
        p_buffer(p_buffer_)
    {
  ...
  struct const_iterator : public std::iterator<std::random_access_iterator_tag, const T>
  {
    ...
    //***************************************************
    const_iterator(const typename ideque_atomic::iterator& other)
      : index(other.index),
      p_deque(other.p_deque),
      p_buffer(other.p_buffer)
    {
  ...
  private:
    ...
    //***************************************************
    const_iterator(difference_type index_, ideque_atomic& the_deque, pointer p_buffer_)
      : index(index_),
        p_deque(&the_deque),
        p_buffer(p_buffer_)
    {
  ...
  //*************************************************************************
  template<typename TIterator>
  typename etl::enable_if<is_iterator<TIterator>::value, iterator>::type
  ...
  /// Assignment operator.
  //*************************************************************************
  ideque_atomic& operator =(const ideque_atomic& rhs)
  {
    if (&rhs != this)
    {
...
protected:
  ...
  /// Constructor.
  //*************************************************************************
  ideque_atomic(pointer p_buffer_, size_t max_size_, size_t buffer_size_)
    : deque_atomic_base(max_size_, buffer_size_),
      p_buffer(p_buffer_)
  {
  }
...
private:
  ...
  ideque_atomic(const ideque_atomic&);
};
...
class deque_atomic : public etlext::ideque_atomic<T>
{
public:
...
public:
  ...
  //*************************************************************************
  deque_atomic()
    : etlext::ideque_atomic<T>(reinterpret_cast<T*>(&buffer[0]), MAX_SIZE, BUFFER_SIZE)
  {
    etlext::ideque_atomic<T>::initialise();
  }
  ...
  /// Copy constructor.
  //*************************************************************************
  deque_atomic(const deque_atomic& other)
    : etlext::ideque_atomic<T>(reinterpret_cast<T*>(&buffer[0]), MAX_SIZE, BUFFER_SIZE)
  {
    if (this != &other)
    {
      etlext::ideque_atomic<T>::assign(other.begin(), other.end());
    }
  }
  ...
  //*************************************************************************
  template <typename TIterator>
  deque_atomic(TIterator begin_, TIterator end_)
    : etlext::ideque_atomic<T>(reinterpret_cast<T*>(&buffer[0]), MAX_SIZE, BUFFER_SIZE)
  {
    etlext::ideque_atomic<T>::assign(begin_, end_);
  }

  //*************************************************************************
  /// Assigns data to the deque.
  //*************************************************************************
  explicit deque_atomic(size_t n, typename etlext::ideque_atomic<T>::parameter_t value = value_type())
    : etlext::ideque_atomic<T>(reinterpret_cast<T*>(&buffer[0]), MAX_SIZE, BUFFER_SIZE)
  {
    etlext::ideque_atomic<T>::assign(n, value);
  }

  //*************************************************************************
  /// Assignment operator.
  //*************************************************************************
  deque_atomic& operator =(const deque_atomic& rhs)
  {
    if (&rhs != this)
    {
      etlext::ideque_atomic<T>::assign(rhs.begin(), rhs.end());
    }
  ...
  void repair()
  {
    ...
    etlext::ideque_atomic<T>::repair(reinterpret_cast<T*>(&buffer[0]));
  }
...
//***************************************************************************
template <typename T>
bool operator ==(const etlext::ideque_atomic<T>& lhs, const etlext::ideque_atomic<T>& rhs)
{
